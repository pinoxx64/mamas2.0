    ...
    if (user) {
      headerContainer.innerHTML = `
              ...
              <div class="modal fade" id="modalPerfil" tabindex="-1" aria-labelledby="modalPerfilLabel" aria-hidden="true">
                ...
                <div class="modal-footer">
                  <button type="button" class="btn btn-primary" data-bs-dismiss="modal" id="guardar">Guardar</button>
      ...
      for (let i = 0; i < userRol.user.length; i++) {
        if (userRol.user[i].rolId == 1) {
          ...
          document.getElementById("botonesCabecera").innerHTML += `
            <button class="btn" id="preguntaCabeceraModalBtn" style="background-color: #ff4040; color: white;">Crear Preguntas</button>
          `
...
document.getElementById("header-container").addEventListener("click", (event) => {
  ...
  if (event.target && event.target.id === "preguntaCabeceraModalBtn") {
    if (user) {
